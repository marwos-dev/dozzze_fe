...
export default function PropertyBanner({ property }: PropertyBannerProps) {
    const images = property.images.length > 0 ? property.images : [property.cover_image];
    ...
    return (
        <div className="mb-8 rounded-2xl overflow-hidden shadow border border-gray-200">
                ...
                {images.map((img, i) => (
                    ...
                    >
                        ...
                        <div className="absolute bottom-4 left-4 text-white z-10">
                            <h1 className="text-2xl md:text-3xl pl-5 text-dozebg1 font-semibold">{property.name}</h1>
                            <p className="text-sm pl-5 text-dozebg1 md:text-base">{property.address || "Dirección desconocida"}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="p-4 bg-greenlight">
                <p className="text-dozegray mb-3">{property.description || "Descripción no disponible"}</p>
