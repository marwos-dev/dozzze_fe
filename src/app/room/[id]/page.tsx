...
export default function RoomDetailPage() {
    ...
    const { property, loading, error } = useSelector((state: RootState) => state.properties);
    const room = property?.rooms?.find(r => String(r.id) === roomId);

    if (loading) return <p className="text-center">Cargando habitación...</p>;
    if (error || !property) return <p className="text-center text-red-500">Error cargando habitacion</p>;
    if (!room) return <p className="text-center text-gray-500">Habitación no encontrada</p>;
    ...
    return (
        <div className="max-w-4xl mx-auto py-10 px-4">
            ...
            <div className="mt-4 flex items-center gap-2 text-gray-800">
                ...
                <span>Capacidad: {room.pax} persona{room.pax > 1 ? 's' : ''}</span>
            </div>
