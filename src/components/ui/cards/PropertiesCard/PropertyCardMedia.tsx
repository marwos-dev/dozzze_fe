...
export default function PropertyCardMedia({ images, coverImage }: PropertyCardMediaProps) {
    ...
    return (
        <div className="flex flex-col w-full md:w-[320px] gap-2">
            ...
            <div className="hidden md:flex gap-2 h-full">
                ...
                ${mainImage === src ? "border-blue-500 ring-2 ring-blue-400" : "border-white"}
              `}
                        >
                            <Image src={src} alt={`Thumbnail ${i + 1}`} fill sizes="70px" className="object-cover" />
                        </button>
                    ))}
                    {extraImagesCount > 0 && (
